/**
 * Customisation Lightbox Module
 * Opens a swipeable photo viewer when clicking customisation cards
 */

export function initCustomLightbox() {
  const lightbox = document.getElementById('customLightbox');
  if (!lightbox) return;

  const dataEl = document.getElementById('customPhotosData');
  if (!dataEl) return;
  ...
  const triggers = document.querySelectorAll('.custom-card[data-custom-index]');
  ...
  function openLightbox(optionIndex) {
    const option = options[optionIndex];
    if (!option || !option.photos || !option.photos.length) return;

    totalPhotos = option.photos.length;
    currentIndex = 0;

    locationEl.textContent = option.description;
    titleEl.textContent = option.name;

    track.innerHTML = option.photos.map((src, i) =>
      `<div class="photo-lightbox-slide">
        <img src="${src}" alt="${option.name} — Photo ${i + 1}" ${i > 0 ? 'loading="lazy"' : ''}>
      </div>`
    ).join('');
  ...
  triggers.forEach(trigger => {
    trigger.addEventListener('click', () => {
      const idx = parseInt(trigger.dataset.customIndex, 10);
      openLightbox(idx);
    });
  });
